	...
	internal class Lang
	{
		...
		public static void setLang()
		{
			...
			for (int i = 0; i < Main.recipe.Length; i++)
			{
				...
				{
					Main.recipe[i].createItem.name = Lang.itemName(Main.recipe[i].createItem.netID);
					Main.recipe[i].createItem.CheckTip();
					for (int j = 0; j < Main.recipe[j].requiredItem.Length; j++)
					{
						Main.recipe[i].requiredItem[j].name = Lang.itemName(Main.recipe[i].requiredItem[j].netID);
